...
namespace View {
    ...
    public class Contact : Gtk.Overlay {
        ...
        private bool has_small_title = false;
        ...
        construct {
            try {
                icon = new Granite.Widgets.Avatar.from_pixbuf (new Pixbuf.from_resource_at_scale ("/com/github/aggalex/contacts/avatar64.svg", 64, -1, true));
            } catch (Error e) {
                critical (@"Failed loading 64px avatar icon: $(e.message)");
            }
            ...
            scroll.get_style_context ().add_class ("chop-bottom-end");
            ...
            var context = new Gtk.Box (Gtk.Orientation.VERTICAL, 0);
            context.pack_start (title_box, false, false, 0);
            context.pack_start (separator_revealer, false, false, 0);
            context.pack_end (scroll, true, true, 0);
            ...
            icon_button.margin_start = 12;

            var icon_revealer = new Gtk.Revealer ();
            icon_revealer.add (icon_button);
            icon_revealer.transition_type = Gtk.RevealerTransitionType.CROSSFADE;
            icon_revealer.show_all ();
            icon_revealer.reveal_child = true;
            icon_revealer.halign = Gtk.Align.START;
            icon_revealer.valign = Gtk.Align.START;
            ...
            var title_box_horizontal_animator = new ValueAnimator (12.0, 100.0);
            title_box_horizontal_animator.value_changed.connect ((margin) => {
                title_box.margin_start = !reverse? 112 - (int) margin: (int) margin;
                return true;
            });
            ...
            scroll.edge_reached.connect ((position) => {
